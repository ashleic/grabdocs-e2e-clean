      ...
      ({ DOC_QUERY, EXPECT_FTO }) => {
        ...
        cy.get('body', { timeout: 30000 }).then(($body) => {
          ...
          if ($searchInputs.length) {
            ...
            cy.log('No search input found. Clicking document title directly.');
          }
        });

        cy.contains(new RegExp(DOC_QUERY, 'i'), { timeout: 30000 })
          .scrollIntoView()
          .click({ force: true });

        cy.contains(/Ask|Doc Chat|Chat with doc|Ask this document|Chat/i, { timeout: 30000 })
          .click({ force: true });

        const question = 'In the Edgesource Employee Handbook, what does "FTO" stand for?';
        ...
        cy.contains(EXPECT_FTO, { timeout: 90000 }).should('be.visible');
      }
    );
  });
});
